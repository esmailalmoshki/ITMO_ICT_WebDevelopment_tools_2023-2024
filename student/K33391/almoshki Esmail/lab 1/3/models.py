...
class Person(DefaultPerson, table=True):
    ...
    books: Optional[List["Book"]] = Relationship(
        link_model=BookOwnership, back_populates='owners')
...
class Book(DefaultBook, table=True):
    ...
    owners: Optional[List[Person]] = Relationship(
        link_model=BookOwnership, back_populates='books')
...
class Request(DefaultRequest, table=True):
    ...
    sender: Optional[Person] = Relationship(
        sa_relationship_kwargs={"foreign_keys": "[Request.sender_id]"})
    receiver: Optional[Person] = Relationship(
        sa_relationship_kwargs={"foreign_keys": "[Request.reciever_id]"})
